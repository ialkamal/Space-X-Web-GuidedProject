...
test("displays error correctly", () => {
  //arrange (render component + setup data)
  const { rerender, getByTestId, queryByTestId } = render(
    <MissionList missions={[]} />
  );

  //act (get that there are no missions yet)
  const error1 = queryByTestId(/error/i);

  //assert (assert that there are no missions yet)
  expect(error1).not.toBeInTheDocument();

  //act (re-render with mission data)
  rerender(<MissionList error="My Error Message" missions={[]} />);

  const error2 = getByTestId(/error/i);
