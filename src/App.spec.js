...
test("Component Successfully renders from api", async () => {
  mockFetchMissions.mockResolvedValueOnce(missionsData);

  const { getByRole, findByText, getAllByTestId } = render(<App />);

  //click on get data btn, fetch message, call api
  const button = getByRole("button", { name: /get data/i });

  //waits for api and waits for api
  userEvent.click(button);

  await findByText(/we are fetching data/i);
  //expect(missionsFetch).toBeInTheDocument();

  //   const missions = findAllByTestId(/mission/i);
  await waitFor(() => {
